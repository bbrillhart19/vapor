...
class SteamClient(Steam):
    ...
    def get_user_details(
        self, user_id: str, fields: list[str] | None = None
    ) -> dict[str, Any]:
        ...
        return {field: user_details.get(field, None) for field in fields}
    ...
    def get_user_owned_games(
        ...
        for game in games["games"]:
            ...
            if "appid" not in game:
                continue
            game_details["appid"] = str(game["appid"])
            for field in fields:
                game_details[field] = game.get(field, None)
            yield game_details
    ...
    def get_user_friends(
        ...
        detailed_friends_list = [
            {field: friend[field] for field in fields} for friend in friends["friends"]
        ]
